...
namespace ChatGPTExtension
{
    ...
    public class ListViewDragDropManager<ItemType> where ItemType : class
    {
        ...
        {
            // Check if the mouse is over the scrollbar. If yes, do not initiate the drag operation.
            ...
            // Conduct a hit test to determine where the mousedown occurred
            var hitTestResult = VisualTreeHelper.HitTest(listView, e.GetPosition(listView));

            // Traverse up the visual tree from the clicked item to find a parent of type ListViewItem
            ...
            if (hitListViewItem != null && isDragHandle)
            {
                int index = listView.Items.IndexOf(hitListViewItem.DataContext);
        ...
        void listView_Drop(object sender, DragEventArgs e)
        {
            ...
            else
            {
                ...
                ConfigurationWindow._dataChanged = true;
